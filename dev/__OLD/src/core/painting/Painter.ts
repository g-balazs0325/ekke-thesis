...
export default abstract class Painter {
  ...
  beginPainting(clientPosition: Vector2): void {
    if (this.cachedFaces)
      throw new PainterError(
        "Painter was trying to begin while not in closed state."
      );
  ...
  endPainting(): void {
    if (!this.cachedFaces)
      throw new PainterError(
        "Painter was trying to close while in closed state."
      );
  ...
  private checkBegin(): void {
    if (!this.cachedFaces)
      throw new PainterError(
        "Painter needs to begin painting before calling this method."
      );
  }
